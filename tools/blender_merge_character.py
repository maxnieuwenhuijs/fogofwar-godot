...
def make_gibs(model_out):
    ...
    # Het INGEBAKKEN WAPEN (tripo_node/musket/sabel/bijl/...) is GEEN
    # lichaamsdeel: bij de dood vliegt de losse wapen-glb al vanaf de hand weg
    # (pawn_view, 16 augustus). Als gib zou hij dubbel vliegen — dat was
    # precies het spook-gib-probleem dat op 15 augustus uit 12 oude bestanden
    # is geschoond. Melee-woorden erbij: big bros komen net als infanterie met
    # hun wapen vast in het model. Zelfde lijst als pawn_view.WAPEN_WOORDEN
    # ("blade" bewust niet: shoulder_blade zou vals matchen). Kale naam
    # (alleen letters), net als het spel: "tripo_node_ab3" -> "triponodeab".
    def kale_naam(n):
        return ''.join(c for c in n.lower() if c.isalpha())
    WAPEN_WOORDEN = ("triponode", "musket", "rifle", "gun", "weapon",
                     "sabre", "saber", "sword", "axe", "lance", "pike",
                     "spear", "cutlass", "scythe", "hatchet", "falchion",
                     "dagger")
